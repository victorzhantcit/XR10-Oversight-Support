...
#pragma warning restore CS0162 // unreachable code detected
		...
		public static int StartFMMicrophoneWebGL(AudioOutputFormat outputFormat, int outputSampleRate, int outputChannels, Action<float[]> callback)
		{
			...
			FMCaptureMicStart_2021_2(_callbackID, callback_OnReceivedRawPCM32IntPtr);

			return _callbackID;
		}

		[AOT.MonoPInvokeCallback(typeof(Action<int, int, IntPtr>))]
		public static void callback_OnReceivedRawPCM32IntPtr(int dictionaryID, int length, IntPtr ptr)
		{
			float[] floatData = FMCoreTools.IntPtrToFloatArray(ptr, length);
			if (dictionary_callbacks.TryGetValue(dictionaryID, out Action<float[]> _targetCallback)) _targetCallback.Invoke(floatData);
		}
		...
		private static Dictionary<int, Action<float[]>> dictionary_callbacks = new Dictionary<int, Action<float[]>>();
