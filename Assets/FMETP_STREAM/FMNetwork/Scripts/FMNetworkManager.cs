...
namespace FMSolution.FMNetwork
{
    ...
    public class FMNetworkManager : MonoBehaviour
    {
        ...
        public void RegisterFMNetworkID(FMNetworkTransformView inputTransformView)
        {
            ...
            {
                DebugLogWarning("reached maximum transform view count: " + maximumTransformViewCount);
                return;
            }

            transformViewDictionary.Add(inputTransformView.GetViewID(), inputTransformView);
            DebugLog("register: view id: " + inputTransformView.GetViewID() + ", total:" + transformViewDictionary.Count);
        ...
        public void UnregisterFMNetworkID(FMNetworkTransformView inputTransformView)
        {
            ...
            DebugLog("unregister: view id: " + inputTransformView.GetViewID() + ", total:" + transformViewDictionary.Count);
...
#pragma warning restore CS0414 // assigned but its value is never used
        ...
        private void UpdateDebugText(bool onNetworkManagerDisabled = false)
        {
            ...
            if (DebugStatus)
            {
                ...
                if (!onNetworkManagerDisabled)
                {
                    ...
                    catch(Exception e) { DebugLogWarning(e.ToString()); }
                }
                else
                {
                    switch (NetworkType)
        ...
        private void Send(byte[] _byteData, FMSendType _type, string _targetIP, bool _reliable = false)
        {
            ...
            {
                DebugLogWarning("Your FMNetwork Type is \"DataStream\", please use StreamData() method instead. This data are ignored and not sent due to mis-matching network type and send method.");
                return;
            }
