...
namespace Inspection.Core
{
    ...
    {
        private ServiceManager _service; // 由 rootUI 負責做服務管理的依賴注入
        private InspectionRootView _inspectionRootUI;
        ...
        public void AssignService(ServiceManager service, InspectionRootView inspectionRootUI)
        {
            _service = service;
            _inspectionRootUI = inspectionRootUI;
        }
        ...
        public void SetDetails(OrderDeviceDto orderDeviceDto, bool inspcetionRejected = false)
        {
            ...
            _datumDateLabel.text = $"數據紀錄 {GetFormatDate(orderDeviceDto.dataTime)}";
        ...
        {
            _inspectionRootUI.RequestPhotoCapture(texture => HandleScreenshotTaken(texture != null, texture));
        }

        private void HandleScreenshotTaken(bool success, string capturedBase64Photo)
        {
            ...
            string tempSns = _service.AddInspPhoto(capturedBase64Photo);
