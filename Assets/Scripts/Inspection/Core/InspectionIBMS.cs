...
namespace Inspection.Core
{
    public class InspectionIBMS
    {
        ...
        private string API_DeviceLifeCycle => $"{SERVER_PORT}/api/Eqpt/DeviceLifeCycle";
        ...
        private InspectionPhotosStorage _photoMap = new InspectionPhotosStorage();
        ...
        public InspectionIBMS(string serverPort, string buildingCode)
        {
            ...
            LocalFileSystem.CheckOrCreateFile(_inspectionPhotosFile, new InspectionPhotosStorage());
            InitPhotoMap();
        }
        ...
        #region Photo
        public async Task<InspectionPhotosStorage> GetLocalPhotosAsync()
            => await LocalFileSystem.GetLocalDataAsync<InspectionPhotosStorage>(_inspectionPhotosFile);

        public void SavePhotoMap(InspectionPhotosStorage photoMap)
            => LocalFileSystem.SaveData(photoMap, _inspectionPhotosFile);

        // API: /api/Eqpt/GetPhoto
        private async Task<string> GetServerPhotoAsync(string photoSn, bool refreshedToken = false)
        {
            List<KeyValue> requestData = new List<KeyValue> {{ new KeyValue("Sn", photoSn) }};
            return await APIHelper.SendServerFormRequestAsync<string>(API_Photo, HttpMethod.GET, requestData, refreshedToken);
        }
        ...
        public async Task ParseAndLoadPhoto(string photoSnsString)
        {
            ...
            for (int i = 0; i < photoSnsArray.Length; i++)
            {
                ...
                {
                    Debug.Log("[*] _photoMap.ContainsPhotoSns");
                    continue;
                }

                string photoBase64 = await GetServerPhotoAsync(cachePhotoSns);

                if (string.IsNullOrEmpty(photoBase64))
                {
                    Debug.Log("[*] server response string.IsNullOrEmpty");
                    continue;
                }

                _photoMap.AddPhoto(cachePhotoSns, photoBase64);
